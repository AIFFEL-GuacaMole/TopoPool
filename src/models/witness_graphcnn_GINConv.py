...
class GraphCNN(nn.Module):
    def __init__(self,
        ...
        # point clouds pooling for nodes - Converting GCNConv to GINConv
        ...
        # point clouds pooling for graphs - Converting GCNConv to GINConv
                ...
                try:
                    ...
                    if len(witnesses_dgm) > 0 and np.isfinite(witnesses_dgm).all():
                        ...
                        PI_witnesses_dgm = torch.FloatTensor(
                            persistence_images(
                                dgm=witnesses_dgm,
                                resolution=[int(np.sqrt(self.PI_dim)),  # make sure to be non-negative
                                            int(np.sqrt(self.PI_dim))],
                                normalization=False
                            ).reshape(1,-1)
                        ).to(self.device)
                    else:
